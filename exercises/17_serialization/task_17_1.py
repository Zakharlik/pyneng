...
def write_dhcp_snooping_to_csv(filenames, output):
    with open(output, "w") as outfile:
        ...
        outline = ['switch','mac','ip','vlan','interface']
        ...
        for file in filenames:
            ...
            with open(file) as f:
                for line in f:
                    outline = [sw_name]
                    record  = re.search('(?P<mac>\S+)\s+(?P<ip>\S+)\s+\d+\s+\S+\s+(?P<vlan>\d+)\s+(?P<intf>\S+)',line)
                    if record:
                        for i in range(len(record.groups())):
                            outline.append(record.group(i+1))
                        writer.writerow(outline)


if __name__ == '__main__':
    write_dhcp_snooping_to_csv(['sw1_dhcp_snooping.txt', 'sw2_dhcp_snooping.txt', 'sw3_dhcp_snooping.txt'], 'outfile.csv')